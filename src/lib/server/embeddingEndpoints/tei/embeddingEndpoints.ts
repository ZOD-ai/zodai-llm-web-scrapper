...
const getModelInfoByUrl = async (url: string, authorization?: string) => {
   ...
   try {
      ...
      logger.debug(
         'Could not get info from TEI embedding endpoint. Using defaults.'
      );
...
export async function embeddingEndpointTei(
   ...
   const { url, model, authorization } =
      embeddingEndpointTeiParametersSchema.parse(input);

   const { max_client_batch_size, max_batch_tokens } = await getModelInfoByUrl(
      url
   );
