...
export async function getTools(
   ...
   const activeConfigTools = toolFromConfigs.filter((el) => {
      if (el.isLocked && el.isOnByDefault && !assistant) {
         return true;
      }
      return (
         preferences?.includes(el._id.toString()) ??
         (el.isOnByDefault && !assistant)
      );
   });
...
async function* callTool(
   ...
   try {
      ...
      await collections.tools.findOneAndUpdate(
         { _id: tool._id },
         { $inc: { useCount: 1 } }
      );
      ...
      MetricsServer.getMetrics().tool.toolUseCountError.inc({
         tool: call.name,
      });
      logger.error('Failed while running tool ' + call.name, error);
...
export async function runTools(
   ...
   const endpointResult = await endpoint({
   ...
   const results = Array.isArray(endpointResult)
      ? endpointResult
      : [endpointResult];
   ...
   for (const output of results) {
      ...
      if (output.generatedText) {
         try {
            const rawCalls = await extractJson(output.generatedText);
            ...
            logger.error('Error while parsing tool calls, please retry', e);
            // error parsing the calls
            throw new Error('Error while parsing tool calls, please retry');
         }
      }
   }
   ...
   const toolContext: BackendToolContext = {
      conv,
      messages,
      preprompt,
      assistant,
   ...
   const toolResults: (ToolResult | undefined)[] = await Promise.all(
      calls.map(async (call) => {
         const generator = callTool(toolContext, tools, call);
         let result: ToolResult | undefined;
         for await (const update of generator) {
            if (
               'status' in update &&
               'call' in update &&
               update.status in ToolResultStatus
            ) {
               result = update as any as ToolResult;
            }
         }
         return result;
      })
...
export async function* runToolsWithStreaming(
...
function externalToToolCall(
   ...
   if (!tool) {
      logger.debug(
         `Model requested tool that does not exist: "${call.tool_name}". Skipping tool...`
      );
      return;
   }
