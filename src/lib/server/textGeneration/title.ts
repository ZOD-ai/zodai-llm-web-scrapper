...
export async function generateTitleForConversation(conv: Conversation) {
...
export async function* streamingGenerateTitleForConversation(
   conv: Conversation
   ...
   try {
      ...
      logger.error(
         `Failed while generating title for conversation: ${JSON.stringify(
            cause,
            null,
            2
         )}`
