...
Current Question: Where is it being hosted?`,
      ...
      { from: 'assistant', content: `weather in Paris ${currentDateTime}` },
      {
         from: 'user',
         content:
   ...
   const webQuery = await generateFromDefaultEndpoint({
      messages: convQuery,
      preprompt: `You are tasked with generating web search queries. Give me an appropriate query to answer my question for google search. Answer with only the query. If context needs current date and time, use the following: ${currentDateTime}`,
      generateSettings: {
         max_new_tokens: 30,
      },
   });
