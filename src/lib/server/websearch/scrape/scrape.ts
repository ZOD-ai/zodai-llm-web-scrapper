...
import { SerializedHTMLElement } from './types';
...
export const scrape = (maxCharsPerElem: number) =>
   ...
   ): Promise<WebSearchScrapedSource | undefined> {
      try {
         if (!source.link) {
            logger.warn('No link found for source', source);
            return undefined;
         }
         ...
         MetricsServer.getMetrics().webSearch.pageFetchDuration.observe(
            Date.now() - startTime
         );

         logger.info('Browsing webpage', source.link);
         return { ...source, page };
      } catch (e) {
         MetricsServer.getMetrics().webSearch.pageFetchCountError.inc();
         logger.error('Error scraping webpage', e);
         return undefined;
      }
   };

export const scrapeWithStreaming = (maxCharsPerElem: number) =>
   async function* (
      source: WebSearchSource
   ): AsyncGenerator<
...
export async function scrapeUrl(url: string, maxCharsPerElem: number) {
   return withPage(url, async (page, res) => {
      ...
      const scrapedOutput = await page
         .evaluate(() => {
            return new Promise((resolve) => {
               setTimeout(() => {
                  ...
                  const hasValidParent = (node: Node) => {
                     return (
                        node.parentElement &&
                        !node.parentElement.isSameNode(document.body)
                     );
                  };

                  const possibleCodeParents = Array.from(
                     document.querySelectorAll('pre, p')
                  );
                  const possibleTableParents = Array.from(
                     document.querySelectorAll('table')
                  );
                  const possibleListParents = Array.from(
                   ...
                   */
                  const findHighestDirectParentOfReadableNode = (
                     node: Node
                  ): HTMLElement => {
                  ...
                  const doesNodePassHeuristics = (node: Node) => {
                     ...
                     const parentNode =
                        findHighestDirectParentOfReadableNode(node);
                     ...
                     if (parentNode && parentNode instanceof Element) {
                        if (
                           barredNodes.some((barredNode) =>
                              barredNode.contains(parentNode)
                           )
                        ) {
                           return false;
                        }
                  ...
                  const getAllReadableNodes = (): NodeWithRect[] => {
                     ...
                     while (treeWalker.nextNode()) {
                        readableNodes.push(
                           treeWalker.currentNode as ReadableNode
                        );
                     }
                     ...
                     for (let i = 0; i < parentsForReadableNodes.length; i++) {
                        ...
                        const hasParentInList = parentsForReadableNodes.find(
                           (otherNode, idx) => {
                              if (i === idx) {
                                 return false;
                              }
                        ...
                        listWithOnlyParents.push(
                           hasParentInList ? hasParentInList : node
                        );
                     }

                     const uniqueParents = Array.from(
                        new Set(listWithOnlyParents)
                     );
                  ...
                  const totalTextLength = (cluster: number[]) => {
                     return cluster
                        .map((t) =>
                           readableNodes[t].node.innerText?.replace(
                              / {2}|\r\n|\n|\r/gm,
                              ''
                           )
                        )
                  ...
                  const getClusterBounds = (cluster: number[]) => {
                     ...
                     const rightMostPoint = Math.max(
                        ...cluster.map(
                           (c) =>
                              readableNodes[c].rect.x +
                              readableNodes[c].rect.width
                        )
                     );
                     const bottomMostPoint = Math.max(
                        ...cluster.map(
                           (c) =>
                              readableNodes[c].rect.y +
                              readableNodes[c].rect.height
                        )
                     );
                     return {
                        // left most element
                  ...
                  const clusterCentrality = (cluster: number[]) => {
                     ...
                     if (
                        bounds.x < centerOfScreen &&
                        bounds.x + bounds.width > centerOfScreen
                     ) {
                        return 0;
                     }
                  ...
                  const percentageTextShare = (
                     ...
                     return round(
                        (totalTextLength(cluster) / totalLength) * 100
                     );
                  };

                  const shouldMergeClusters = (
                     clusterA: number[],
                     clusterB: number[]
                  ) => {
                     ...
                     const higherCluster =
                        clusterABounds.y < clusterBBounds.y
                           ? clusterABounds
                           : clusterBBounds;
                     const lowerCluster =
                        clusterABounds.y < clusterBBounds.y
                     ...
                     const yGap =
                        lowerCluster.y -
                        (higherCluster.y + higherCluster.height);

                     if (approximatelyEqual(yGap, 0, 100)) {
                        return true;
                     }
                  };
                  ...
                  const findCriticalClusters = (clusters: number[][]) => {
                     ...
                     const dominantCluster =
                        clusterWithMetrics[0]?.percentageTextShare > 60;
                     if (dominantCluster) {
                        return [clusterWithMetrics[0].cluster];
                     }
                     ...
                     const sortedClusters = clusterWithMetrics.sort((a, b) => {
                        ...
                        const adjustedTextShareA =
                           a.percentageTextShare * penaltyForA;
                        const adjustedTextShareB =
                           b.percentageTextShare * penaltyForB;
                     ...
                     const totalTextShareOfLargeClusters =
                        largeTextShareClusters.reduce(
                     ...
                     for (const cluster of sortedClusters) {
                        ...
                        if (cluster.percentageTextShare < 2) {
                           continue;
                        }
                  ...
                  function serializeNode(
                     node: Node
                  ): SerializedHTMLElement | string {
                     if (node.nodeType === 1) {
                        return serializeHTMLElement(node as Element);
                  ...
                  const elements = filteredNodes
                     .filter(
                        (node, idx, nodes) =>
                           !nodes
                              .slice(idx + 1)
                              .some((otherNode) => node.node === otherNode.node)
                     )
                     .map<SerializedHTMLElement>(({ node }) =>
                        serializeHTMLElement(node)
                     );
                  ...
                  // Timeout needed to wait for the page to load
               }, 2000);
            });
         })
         .then(({ elements, ...parsed }: any) => {
            logger.info(`Scraped elements length: ${elements.length}`);
            logger.info(`Scraped metadata: ${JSON.stringify(parsed)}`);
