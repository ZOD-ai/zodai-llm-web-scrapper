...
export function spatialParser() {
   ...
   const hasValidParent = (node: Node) => {
      return (
         node.parentElement && !node.parentElement.isSameNode(document.body)
      );
   };
   ...
   const doesNodePassHeuristics = (node: Node) => {
      ...
      if (parentNode && parentNode instanceof Element) {
         if (
            barredNodes.some((barredNode) => barredNode.contains(parentNode))
         ) {
            return false;
         }
      }
   ...
   const totalTextLength = (cluster: number[]) => {
      return cluster
         .map((t) =>
            readableNodes[t].node.innerText?.replace(/ {2}|\r\n|\n|\r/gm, '')
         )
         .join('').length;
   };
