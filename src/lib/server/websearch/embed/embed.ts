...
export async function findContextSources(
   ...
   const sourcesMarkdownElems = sources.map((source) =>
      flattenTree(source.page.markdownTree)
   );
   ...
   for (const embedding of topEmbeddings) {
      ...
      if (!selectedMarkdownElems.has(elem)) {
         ...
         totalChars += elem.content?.length ?? 0;
      }
      // Add element's parent (header)
      if (elem.parent && !selectedMarkdownElems.has(elem.parent)) {
         selectedMarkdownElems.add(elem.parent);
         totalChars += elem.parent.content?.length ?? 0;
      }

      if (totalChars > SOFT_MAX_CHARS) {
         break;
      }
      if (totalChars > MIN_CHARS && embedding.distance > 0.25) {
         break;
      }
   }
   ...
   MetricsServer.getMetrics().webSearch.embeddingDuration.observe(
      Date.now() - startTime
   );
