...
import { NonStreamingContentGenerationContext } from '../textGeneration/types';
...
export const editableToolSchema = z
   .object({
      ...
      baseUrl: z.union([
         ...
         z
            .string()
            .regex(
               /^https:\/\/huggingface\.co\/spaces\/[a-zA-Z0-9-]+\/[a-zA-Z0-9-]+$/
            )
   ...
   .transform((tool) => ({
      ...
      outputComponent: ToolOutputComponents.parse(
         tool.outputComponent.split(';')[1]
      ),
   }));
...
export function getCallMethod(tool: Omit<BaseTool, 'call'>): BackendCall {
   return async function* (params, ctx, uuid) {
      ...
      const ipToken = ctx.ip && (await getIpToken(ctx.ip, ctx.username));
      ...
      const inputs = tool.inputs.map(async (input) => {
         if (input.type === 'file' && input.paramType !== 'required') {
            throw new Error(
               'File inputs are always required and cannot be optional or fixed'
            );
         }
         ...
         if (input.paramType === 'fixed') {
            ...
            if (input.type === 'file') {
               ...
               const blob = await downloadFile(file.value, ctx?.conv?._id)
                  .then((file) =>
                     fetch(`data:${file.mime};base64,${file.value}`)
                  )
                  .then((res) => res.blob())
                  .catch((err) => {
                     throw Error(
      ...
      if (!path || !type) {
         throw new Error(
            `Tool output type ${tool.outputComponent} is not supported`
         );
      }
                  ...
                  await Promise.all(
                     arrayedOutput.map(async (output, idx) => {
                        await fetch(output)
                           .then((res) => res.blob())
                           .then(async (blob) => {
                              const { ext, mime } = (await fileTypeFromBlob(
                                 blob
                              )) ?? { ext: 'octet-stream' };
...
export const toolFromConfigs = configTools.parse(
   JSON5.parse(env.TOOLS)
) satisfies ConfigTool[];
