...
export type ImageProcessor<TMimeType extends string = string> = (
   file: MessageFile
) => Promise<{
   image: Buffer;
   mime: TMimeType;
...
export function makeImageProcessor<TMimeType extends string = string>(
   ...
   return async (file) => {
      ...
      const outputMime = chooseMimeType(
         supportedMimeTypes,
         preferredMimeType,
         mime,
         {
...
const isOutputFormat = (
   format: string
): format is (typeof outputFormats)[number] =>
...
function chooseMimeType<T extends readonly string[]>(
   ...
   const smallestMime = mimesBySizeDesc
      .reverse()
      .find((m) => supportedMimes.includes(m));
...
export function chooseImageSize({
   ...
   do {
      const estimatedSize = estimateImageSizeInBytes(
         mime,
         selectedWidth,
         selectedHeight
      );
 ...
 **/
function estimateImageSizeInBytes(
   mime: string,
   width: number,
   height: number
): number {
