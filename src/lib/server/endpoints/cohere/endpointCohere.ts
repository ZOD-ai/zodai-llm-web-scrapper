...
export async function streamingEndpointCohere(
   input: z.input<typeof endpointCohereParametersSchema>
): Promise<StreamingEndpoint> {
   const { apiKey, clientName, model, raw } =
   ...
   try {
      ...
      throw new Error(`Failed to import cohere-ai: ${JSON.stringify(e)}`);
   }
   ...
   return async ({
      ...
      tools = tools
         ...
         .map((tool) => ({ ...tool, name: tool.name.replace('-', '_') }));
      ...
      return (async function* () {
         ...
         if (raw) {
            ...
            stream = await cohere
               .chatStream({
                  ...
                  message:
                     formattedMessages[formattedMessages.length - 1].message,
                  preamble: system,
                  p: parameters?.top_p,
                  k: parameters?.top_k,
                  maxTokens: parameters?.max_new_tokens,
                        ...
                        ? toolResults?.map((toolResult) => {
                             if (toolResult.status === ToolResultStatus.Error) {
                                return {
                                   ...
                                   outputs: [{ error: toolResult.message }],
                                };
                             }
                             return {
                                call: toolResult.call,
               ...
               .catch(async (err: { body: Readable }) => {
                  ...
                  throw Error(
                     `Cohere error: ${message}. Original error: ${JSON.stringify(
                        err
                     )}`
                  );
               });
         ...
         for await (const output of stream) {
            if (output.eventType === 'text-generation') {
               ...
               if (
                  ['ERROR', 'ERROR_TOXIC', 'ERROR_LIMIT'].includes(
                     output.finishReason
                  )
               ) {
               ...
               yield {
                  ...
                  generatedText: output.response.text,
                  details: null,
               };
            }
         }
