...
export function streamingEndpointGenAI(
   input: z.input<typeof endpointGenAIParametersSchema>
): StreamingEndpoint {
   const { model, apiKey, safetyThreshold, multimodal } =
      endpointGenAIParametersSchema.parse(input);
   ...
   return async ({ messages, preprompt, generateSettings }) => {
      ...
      const genAIMessages = await Promise.all(
         messages.map(
            async ({
               ...
               content,
               files,
            }: Omit<Message, 'id'>): Promise<Content> => {
               return {
                  role: from === 'user' ? 'user' : 'model',
                  parts: [
                     ...(await Promise.all(
                        (files ?? []).map((file) =>
                           fileToImageBlock(file, multimodal.image)
                        )
                     )),
                     { text: content },
                  ],
      ...
      return (async function* () {
         ...
         for await (const data of result.stream) {
            ...
            const firstPart = candidate.content.parts.find(
               (part) => 'text' in part
