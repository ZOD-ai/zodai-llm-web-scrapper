...
export function streamingEndpointOllama(
   input: z.input<typeof endpointOllamaParametersSchema>
): StreamingEndpoint {
   const { url, model, ollamaName } =
      endpointOllamaParametersSchema.parse(input);
   ...
   return async ({
      ...
      if (!tags.models.some((m: { name: string }) => m.name === ollamaName)) {
         ...
         throw new Error(
            'Currently pulling model from Ollama, please try again later.'
         );
      }
