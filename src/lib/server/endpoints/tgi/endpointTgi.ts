...
import type { StreamingEndpoint, EndpointMessage } from '../endpoints';
import {
...
export function streamingEndpointTgi(
   input: z.input<typeof endpointTgiParametersSchema>
): StreamingEndpoint {
   ...
   return async ({
      ...
      const messagesWithResizedFiles = await Promise.all(
         messages.map((message) =>
            prepareMessage(Boolean(isMultimodal), message, imageProcessor)
         )
      );
...
async function prepareMessage(
   ...
   const files = await Promise.all(
      message.files?.map(imageProcessor) ?? [whiteImage]
   );
