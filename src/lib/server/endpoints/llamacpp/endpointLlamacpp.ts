...
export function streamingEndpointLlamacpp(
   input: z.input<typeof endpointLlamacppParametersSchema>
): StreamingEndpoint {
   const { baseURL, url, model } =
      endpointLlamacppParametersSchema.parse(input);
   return async ({
      messages,
      preprompt,
      continueMessage,
               ...
               if (jsonString.startsWith('data: ')) {
                  ...
                  if (data.content || data.stop) {
                     ...
                     const output: TextGenerationStreamOutput = {
                        ...
                        generatedText: data.stop ? generatedText : null,
                        details: null,
                     };
                     if (data.stop) {
                        stop = true;
