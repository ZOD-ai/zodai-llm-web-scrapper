...
export class Database {
   ...
   private constructor() {
      ...
      this.client.db(
         env.MONGODB_DB_NAME + (process.env.NODE_ENV === 'test' ? '-test' : '')
      );
   ...
   public getCollections() {
      ...
      const conversationStats = db.collection<ConversationStats>(
      ...
      const abortedGenerations =
         db.collection<AbortedGeneration>('abortedGenerations');
      ...
      const migrationResults =
         db.collection<MigrationResult>('migrationResults');
   ...
   private initDatabase() {
      ...
      assistants
         .createIndex({ featured: 1, userCount: -1 })
         .catch((e) => logger.error(e));
      assistants
      ...
      assistantStats
         ...
         .createIndex(
            { 'date.span': 1, 'date.at': 1, assistantId: 1 },
            { unique: true }
         )
         .catch((e) => logger.error(e));
...
export const collections = {} as unknown as ReturnType<
   typeof Database.prototype.getCollections
>;
